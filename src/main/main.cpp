...
// 定义音频放大模块的I2S引脚定义
#define I2S_DOUT 27//25 // DIN引脚
#define I2S_BCLK 26 // BCLK引脚
#define I2S_LRC 25//27  // LRC引脚
...
// 选哪个模型，就填哪个模型的参数
// 豆包大模型的参数
String model1 = "ep-20241009001227-4kjgd";//"ep-20241005203935-kr4mw";   // 在线推理接入点名称，必填
const char* doubao_apiKey = "62f3c745-405f-4b48-849d-9dc62d17e0b4";     // 火山引擎API Key，必填
...
// 讯飞stt和大模型服务的参数
String APPID = "d99e271c";                             // App ID,必填
String APISecret = "NjllZWUwYTc4Mjg2ZDU1ZWY5YzE5ZjVl"; // API Secret，必填
String APIKey = "1422b43e713d5de0f4bafc012207e377";    // API Key，必填
String appId1 = APPID;
    ...
    else
    {
        ...
        if (jsonDocument["data"]["status"] == 2)
        {
            ...
            if (await_flag == 1)
            {
                ...
                if( (askquestion.indexOf("坤坤") > -1 || askquestion.indexOf("小白") > -1 || askquestion.indexOf("丁真") > -1 || askquestion.indexOf("九歌") > -1 || askquestion.indexOf("你好") > -1) )
                {
                    await_flag = 0;     //退出待机状态
                    start_con = 1;      //对话开始标识
                    Answer = "喵~我在的，主人。";
